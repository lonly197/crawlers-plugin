...
(function($) {
    ...
    $.extend(HighlighterCls.prototype, {
        ...
        createNew: function(id) {
            ...
            this.frame.left = $(
                '<div class="_20p_frame _20p_frame_side' +
                    className +
                    '"></div>'
            ).appendTo($("body"));
            this.frame.right = $(
            ...
            this.frame.center = $(
                '<div class="_20p_frame _20p_frame_center' +
                    className +
                    '"></div>'
            ).appendTo($("body"));

            $.each(this.frame, function(key, value) {
                var sideStyles = $.extend(
                    self.options.styles.defaults,
                    self.options.styles.sides
                );
                value.css(sideStyles);
                if (key == "center") {
                    var centerStyles = $.extend(
                        self.options.styles.defaults,
                        self.options.styles.center
                    );
            ...
            $.each(this.frame, function(key, value) {
                if (runtime_options.bgcolor) {
                    ...
                    if (key == "center") {
                        ...
                        value.css(
                            "backgroundColor",
                            self.options.styles.sides.backgroundColor ||
                                (self.options.styles.defaults.backgroundColor ||
                                    "")
                        );
                    }
                }
                ...
                if (
                    runtime_options.allowEventsOnAllSides == true ||
                    (key == "center" && runtime_options.hole == true)
                ) {
        ...
        setShineElements: function(overEl) {
            var ownerIframeDoc = this.ownerIframe
                ? this.ownerIframe.contentDocument
                : null;
            ...
            if (!$.isEmptyObject(element)) {
                ...
                if (this.v_offset) {
                    offset = {
                        left: this.v_offset.left,
                        top: this.v_offset.top
                    };
                    dimensions = {
                        height: this.v_offset.height,
                        width: this.v_offset.width
                    };
                }
                ...
                this.frame.left
                    .add(this.frame.right)
                    .css("height", dimensions.height);

                this.frame.right.css("left", offset.left + dimensions.width);
                this.frame.right.css(
                    "width",
                    viewportWidth - (offset.left + dimensions.width)
                );
                ...
                this.frame.top
                    .add(this.frame.bottom)
                    .css("width", viewportWidth);
                this.frame.bottom.css("top", dimensions.height + offset.top);
                this.frame.bottom.css(
                    "height",
                    viewportHeight - (offset.top + dimensions.height)
                );
                ...
                this.frame.center.css({
                    ...
                    width:
                        dimensions.width + this.options.centerborderWidth * 2,
                    height:
                        dimensions.height + this.options.centerborderWidth * 2
                });
            }
        ...
        setFrameElements: function(overEl) {
            ...
            if (!$.isEmptyObject(element)) {
                ...
                this.frame.left.css(
                    "left",
                    offset.left - this.options.borderWidth
                );
                this.frame.left.add(this.frame.right).css("top", offset.top);
                this.frame.left
                    .add(this.frame.right)
                ...
                this.frame.top.css(
                    "top",
                    offset.top - this.options.borderWidth
                );
                this.frame.top
                    .add(this.frame.bottom)
                    .css("left", offset.left - this.options.borderWidth);
                this.frame.top
                    .add(this.frame.bottom)
                    .css(
                        "width",
                        dimensions.width + 2 * this.options.borderWidth
                    );
                this.frame.bottom.css("top", dimensions.height + offset.top);
